<template>
  <div class="card">
    <div class="card-body">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">Générateur de binômes/trinômes</h3>
      ...
      <div class="space-y-6">
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th v-if="algorithm === 'balanced'" class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Niveau</th>
                  <th v-if="algorithm === 'mixed'" class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Genre</th>
                </tr>
              </thead>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            >
              <option value="binomes">Binômes (2 élèves)</option>
              <option value="trinomes">Trinômes (3 élèves)</option>
              <option value="custom">Taille personnalisée</option>
            </select>
          </div>

          <div v-if="groupType === 'custom'">
            <label class="block text-sm font-medium text-gray-700 mb-2">Taille des groupes</label>
            <input 
              v-model.number="customGroupSize" 
              type="number" 
              min="2" 
              max="6" 
              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Algorithme</label>
            <select 
              v-model="algorithm" 
              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            >
              <option value="random">Aléatoire</option>
              <option value="balanced">Équilibré par niveau</option>
              <option value="mixed">Mixte hétérogène</option>
            </select>
          </div>
        </div>
        ...
        <!-- Results -->
        <div v-if="generatedGroups.length > 0" class="mt-8">
          <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex space-x-2">
              ...
              >
                <ArrowPathIcon v-if="isExporting === 'excel'" class="w-4 h-4 animate-spin mr-1" />
                <DocumentArrowDownIcon v-else class="w-4 h-4 mr-1" />
                Excel
              </button>
              <button 
                @click="exportPdf" 
              ...
              >
                <ArrowPathIcon v-if="isExporting === 'pdf'" class="w-4 h-4 animate-spin mr-1" />
                <DocumentIcon v-else class="w-4 h-4 mr-1" />
                PDF
              </button>
            </div>
          </div>
