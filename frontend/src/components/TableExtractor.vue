    ...
    <div class="card-body">
      ...
      <div class="space-y-6">
        ...
        <div @drop="handleDrop" @dragover.prevent @dragenter.prevent
          ...
          :class="{ 'border-secondary-400 bg-secondary-50': isDragOver }" @dragenter="isDragOver = true"
          ...
          <input ref="fileInput" type="file" multiple accept="image/*,.pdf" @change="handleFileSelect" class="hidden">
        ...
        <div v-if="selectedFiles.length > 0" class="bg-gray-50 p-4 rounded-lg">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label class="flex items-center space-x-2">
              <input v-model="preserveFormatting" type="checkbox"
            ...
            <label class="flex items-center space-x-2">
              <input v-model="detectHeaders" type="checkbox"
                class="rounded border-gray-300 text-secondary-600 focus:ring-secondary-500">
              <span class="text-sm text-gray-700">Détecter les en-têtes</span>
            </label>

            <label class="flex items-center space-x-2">
              <input v-model="mergeMultipleTables" type="checkbox"
                class="rounded border-gray-300 text-secondary-600 focus:ring-secondary-500">
            ...
            <div class="flex items-center space-x-2">
              ...
              <select v-model="ocrLanguage" class="border border-gray-300 rounded px-2 py-1 text-sm">
        ...
        <div v-if="selectedFiles.length > 0" class="flex justify-end">
          <button @click="extractTables" :disabled="isProcessing"
...
const extractTables = async () => {
  ...
  try {
    ...
    formData.append('conversion_type', 'Autres listes')
    formData.append('image', selectedFiles.value[0]) // si plusieurs fichiers, tu peux les boucler

    const response = await fetch('http://ocr_laravel.test/api/excel-generate', {
      method: 'POST',
      body: formData
    })
    ...
    // Si le back renvoie directement le fichier Excel en réponse (Content-Disposition: attachment)
    const blob = await response.blob()
    const url = window.URL.createObjectURL(blob)

    const a = document.createElement('a')
    a.href = url
    a.download = 'tableau_converti.xlsx' // ou récupéré dynamiquement selon le backend
    a.click()
    window.URL.revokeObjectURL(url)
  } catch (error) {
    console.error('Erreur:', error)
    alert('Une erreur est survenue pendant l\'extraction.')
  } finally {
    isProcessing.value = false
  }
}
